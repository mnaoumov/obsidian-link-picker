...
- **A suite that presses a key is desktop-only.** The harness sends keys through Electron's input API, which Android does not have. That is the harness's wall rather than the plugin's, and each such file records it (G97). A behavior that can be driven by clicking is `*.cross-platform.` and must pass on both.
