 ...
 *
 * **The waiting happens in NODE, and each closure below is milliseconds of DOM reading.** This file
 * declared SEVEN waits in one `evalInObsidian` closure against a ~30s transport cap, the worst in the
 * repo: split evenly each got four seconds, which is not enough for a picker to open on a cold phone.
 * `pollInObsidian` is what makes the budget real: each poll is its own short eval, and `until` runs in
 * Node, where it can close over the names this test already holds.
 */
...
describe('The `Folders only` control', () => {
  it('leaves only the folders, and the way out', async () => {
    ...
    await pollInObsidian({
      ...
      poll({ app, folderName: folder, sourcePath: path, stamp: suffix }): boolean {
        return app.vault.getFileByPath(`${folder}/Note-${suffix}.md`) !== null
          && app.vault.getFolderByPath(`${folder}/Nested-${suffix}`) !== null
          && app.vault.getFileByPath(path) !== null;
      },
      async start({ app, folderName: folder, lib: { createNote }, sourcePath: path, stamp: suffix }): Promise<void> {
        await app.vault.createFolder(folder);
        await app.vault.createFolder(`${folder}/Nested-${suffix}`);
        await createNote({
          content: '# Note\n',
          path: `${folder}/Note-${suffix}.md`
        });
        // The source note is deliberately empty, so a content read-back would prove nothing.
        await app.vault.create(path, '');
      },
      timeoutInMilliseconds: WAIT_TIMEOUT_IN_MILLISECONDS,
      timeoutMessage: 'the staged folder never appeared in the vault',
    ...
    // The absence is HALF the predicate, never the whole of it.
    // Polling for something that is already gone accepts before the thing being waited for has had a chance to happen.
    // Here the note is present until the toggle lands, so the predicate really does start out false.
    await pollRows(
      'the notes were never hidden',
      (rows: string[]): boolean => rows.some((text) => text.includes('Nested-')) && rows.every((text) => !text.includes('Note-'))
    );
    const rowsAfterToggle = await readRows();

    await clickControl('Folders only');
    await pollRows('the notes never came back', (rows: string[]): boolean => rows.some((text) => text.includes('Note-')));
    await chooseRow('Note-');
    await pollPickerClosed('the picker never closed on the pick');

    expect(rowsBeforeToggle.join('\n')).toContain('Note-');
