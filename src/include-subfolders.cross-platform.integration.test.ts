 ...
 * never each get the budget they declared. `pollInObsidian` is what makes the budget real: each poll is
 * its own short eval, and `until` runs in Node, where it can close over the names this test already holds.
 */
...
describe('The `Subfolders` control', () => {
  it('reaches into subfolders, which are otherwise left alone', async () => {
    ...
    await pollInObsidian({
      input: { deepName, folderName, sourcePath, stamp },
      poll({ app, deepName: deep, folderName: folder, sourcePath: path, stamp: suffix }): boolean {
        return app.vault.getFileByPath(`${folder}/${deep}/Buried-${suffix}.md`) !== null && app.vault.getFileByPath(path) !== null;
      },
      async start({ app, deepName: deep, folderName: folder, lib: { createNote }, sourcePath: path, stamp: suffix }): Promise<void> {
        await app.vault.createFolder(folder);
        await app.vault.createFolder(`${folder}/${deep}`);
        await createNote({
          content: '# Buried\n',
          path: `${folder}/${deep}/Buried-${suffix}.md`
        });
    ...
    await pollRows('the folder never opened', (rows: string[]): boolean => rows.some((text) => text.includes(deepName)));
    const rowsBeforeToggle = await readRows();

    await clickControl('Subfolders');
    await pollRows('the buried note was never reached', (rows: string[]): boolean => rows.some((text) => text.includes('Buried-')));
    const rowsAfterToggle = await readRows();

    await chooseRow('Buried-');
